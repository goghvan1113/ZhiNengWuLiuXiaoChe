...
void Timer7_Init(u16 arr,u16 psc)
{
    ...
    TIM_TimeBaseInit(TIM7,&TIM_TimeBaseInitStructure);//初始化定时器4
    
    TIM_ITConfig(TIM7,TIM_IT_Update,ENABLE); //允许定时器4更新中断
    TIM_Cmd(TIM7,ENABLE); //使能定时器4
 
  NVIC_InitStructure.NVIC_IRQChannel = TIM7_IRQn;//外部中断4
